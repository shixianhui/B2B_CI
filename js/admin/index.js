$(document).ready(function() {
	...
	function tab(tabid){
		...
		$("#jifen").removeClass("selected");
		$("#payservice").removeClass("selected");
		$("#zffs").removeClass("selected");
		$("#gwlc").removeClass("selected");
		$("#discount").removeClass("selected");
		$("#score_exchange").removeClass("selected");
		$("#fuwuxieyi").removeClass("selected");
		$("#basics").hide();				
		$("#advanceds").hide();
		$("#appAdvanceds").hide();
		$("#kefus").hide();
    ...
    $("#select_pay_mode").change(function(){
        $ids = "";
		...
		$.post(base_url+"admincp.php/"+controller+"/pay_mode", 
				{	"ids": $ids.substr(0, $ids.length - 1),
			        "pay_mode": $pay_mode
				},
				function(res){
    ...
    $("#upload_image").click(function(){
    	...
    	//window.open(base_url+"admincp.php/upload/index/"+model+"/"+parseImagePath, "upload", "top=100, left=200, width=800, height=400, scrollbars=1, resizable=yes");
	    var d = dialog({
			fixed: true,
		    title: '上传图片：',
    ...
    $("#upload_file").click(function(){
    	...
    	window.open(base_url+"admincp.php/upload/uploadFile/"+parseFilePath, "uploadFile", "top=100, left=200, width=800, height=400, scrollbars=1, resizable=yes");
    });   
    ...
    $("#upload_movie").click(function(){
    	...
    	var parseFilePath = filePath.replace(/\//g, ":");
    	parseFilePath = parseFilePath.replace(/\./g, "_");
    	window.open(base_url+"admincp.php/upload/uploadMovie/"+parseFilePath, "uploadFile", "top=100, left=200, width=800, height=400, scrollbars=1, resizable=yes");
    });
    ...
    $("#batch_upload_image").click(function(){
    	...
    	//window.showModalDialog(base_url+"admincp.php/upload/batch_upload/"+model+"/"+parseImagePathIds, "upload", "dialogTop:100px;dialogLeft:200px;dialogWidth=800px;dialogHeight=400px;resizable=yes;scroll=yes;");
    	var d = dialog({
			fixed: true,
		    title: '批量上传图片：',
		...
		d.show();
    	//window.open(base_url+"admincp.php/upload/batch_upload/"+model+"/"+parseImagePathIds, "upload", "top=100, left=200, width=800, height=400, scrollbars=1, resizable=yes");
        //window.onfocus=function (){result.focus();};
        //window.onclick=function (){result.focus();};
    });
    //select image, open new window
    $("#select_image").click(function(){
    	window.open(base_url+"admincp.php/upload/select", "select", "top=100, left=200, width=800, height=400, scrollbars=1, resizable=yes");
    });   
    //select movie, open new window
    $("#select_movie").click(function(){
    	window.open(base_url+"admincp.php/upload/selectMovie", "selectMovie", "top=100, left=200, width=800, height=400, scrollbars=1, resizable=yes");
    });
    ...
    $("#cut_image").click(function(){
    	...
    	//window.open(base_url+"admincp.php/upload/cutpicture/"+model+"/"+cutImagePath, "cutpic", "top=100, left=200, width=800, height=400, scrollbars=1, resizable=yes");
    	var d = dialog({
			fixed: true,
		    title: '裁剪图片：',
		    content: '<iframe frameborder="0" src="'+base_url+'admincp.php/upload/cutpicture/'+model+'/'+cutImagePath+'" width="680" height="400"></iframe>'
		});
		d.show();
    });
    ...
    if ($menuType !=null && $menuType !="" && $menuType == 3) {
		$("#link_url").show();
	    $("#selectModelTr").hide();
	} else if ($menuType !=null && $menuType !="" && $menuType == 2) {
		$("#model").get(0).options[5].selected = true;
		$("#selectModelTr").hide();
	}
    //delete menu
    //menu sort
