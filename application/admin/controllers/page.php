...
class Page extends CI_Controller {
	...
	public function __construct() {
		...
		$this->_tool = $this->load->view("element/save_list_tool", array('table' => $this->_table, 'parent_title' => '网站管理', 'title' => '文章'), TRUE);
		//获取表对象
	...
	public function index($clear = 0) {
	    ...
	    $uri_sg = base_url().'admincp.php/'.$this->uri->segment(1).$uri_2."/{$clear}";
	    ...
	    if (! empty($ids)) {
	        ...
	        foreach ($itemList as $key=>$value) {
	            if ($systemInfo['client_index']) {
			    	$url = base_url()."{$systemInfo['client_index']}/{$value['table']}/index/{$value['category_id']}/{$value['id']}";
			    } else {
			    	$url = base_url()."{$systemInfo['client_index']}{$value['table']}/index/{$value['category_id']}/{$value['id']}";
			    }
