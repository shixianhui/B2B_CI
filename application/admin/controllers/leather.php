...
class Leather extends CI_Controller {
	...
	public function __construct() {
		...
		//快捷方式
		$this->_tool = $this->load->view("element/save_list_tool", array('table'=>$this->_table, 'parent_title'=>'商品管理', 'title'=>'皮革'), TRUE);
		//获取表对象
	...
	public function save($id = NULL) {
		...
		if ($_POST) {
		    if ($id != NULL) {
		    	$leather_name = trim($this->input->post('leather_name', TRUE));
				...
				$leather_name_arr = explode("|", $leather_name);
				foreach ($leather_name_arr as $key=>$value) {
					$fields = array(
				          'sort'=>$sort+$key,
						  'tag'=>         $tag,
				          'leather_name'=>trim($value)
				          );
			         if ($this->tableObject->save($fields)) {
			             $i++;
			         }
				}
				if (count($leather_name_arr) == $i) {
				    printAjaxSuccess($prfUrl);
				} else {
				    printAjaxError("操作失败！");
				}
