...
class Market extends CI_Controller {
	...
	public function __construct() {
		...
		$this->_table = $this->uri->segment(1);
		//快捷方式
		$this->_tool = $this->load->view("element/save_list_tool", array('table' => $this->_table, 'parent_title' => '系统设置', 'title' => '商场'), TRUE);
		//获取表对象
	...
	public function index($clear = 0, $page = 0) {
		...
		if ($_POST) {
		    ...
		    $site_category_1 = $this->input->post('site_category_1', TRUE);
		    ...
		    if (! empty($site_category_1) ) {
		    	$strWhere .= " and {$this->_table}.site_category_1 = '{$site_category_1}'";
		    }
		    if ($display != "") {
		...
		$site_list = $this->Site_model->gets('*', array('parent_id'=>0));
		...
		foreach ($item_list as $key=>$value) {
		    ...
		    $site_info = $this->Site_model->get('name', array('id'=>$value['site_category_1']));
		    if ($site_info) {
		    	$item_list[$key]['city_name'] = $site_info['name'];
		    } else {
		    	$item_list[$key]['city_name'] = '';
		    }
		}
	...
	public function save($id = NULL) {
		...
		if ($_POST) {
			...
			$location_txt = unhtml($this->input->post('location_txt'));
			...
			$fields = array(
					  ...
					  'introduce'=>       $introduce,
					  'batch_path_ids_top'=>     $this->input->post('batch_path_ids_top', TRUE),
					  'batch_path_ids'=>         $this->input->post('batch_path_ids', TRUE),
					  'batch_path_ids_bottom'=>  $this->input->post('batch_path_ids_bottom', TRUE)
			          );
