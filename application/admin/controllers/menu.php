...
class menu extends CI_Controller {
    ...
    public function main() {
        $this->load->view("{$this->_table}/main");
    }

    public function drag() {
        $this->load->view("{$this->_table}/drag");
    }
    ...
    public function top() {
        ...
        $this->load->view("{$this->_table}/top", $data);
    }
    ...
    public function menus() {
        ...
        $this->load->view("{$this->_table}/menu", array('patternList' => $patternList));
    }

    public function menuList() {
        checkPermission("{$this->_table}_menuList");
        ...
        $layout = array(
            'title' => $this->_title,
            'content' => $this->load->view($this->_table . '/menu_list', $data, TRUE)
        );
    ...
    public function save($childId = 0, $id = NULL) {
        ...
        $prfUrl = $this->session->userdata("{$this->_table }RefUrl") ? $this->session->userdata("{$this->_table }RefUrl") : base_url() . "admincp.php/{$this->_table}/menulist";
        if ($_POST) {
            ...
            $brand_ids = $this->input->post('brand_ids', TRUE);
            $product_category_ids = $this->input->post('product_category_ids', TRUE);
            if ($brand_ids) {
        ...
        $product_category_list = $this->Product_category_model->gets(array('store_id'=>0, 'parent_id'=>0));
        $brand_arr = array();
        $product_category_arr = array();
        if ($menuInfo && $menuInfo[0]) {
        	if ($menuInfo[0]['brand_ids']) {
        		$brand_arr = explode(',', $menuInfo[0]['brand_ids']);
        	}
        	if ($menuInfo[0]['product_category_ids']) {
        		$product_category_arr = explode(',', $menuInfo[0]['product_category_ids']);
        	}
        }
        ...
        $data = array(
        	...
        	'brand_arr'=>$brand_arr,
        	'product_category_arr'=>$product_category_arr,
            'prfUrl' => $prfUrl
        );
        $layout = array(
            'tool' => $this->_tool,
    ...
    public function sort() {
        checkPermissionAjax("{$this->_table}_sort");
        if ($_POST) {
