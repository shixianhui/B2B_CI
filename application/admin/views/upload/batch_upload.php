...
$(function() {
	$('#batch_image_upload').uploadify({
		...
		'onUploadSuccess' : function(file, data, response) {
			...
			if (json.success) {
				<?php if ($id) { ?>
				window.parent.document.getElementById("batch_path_ids_<?php echo $id; ?>").value +=""+json.data.id+"_";
				<?php } else { ?>
				//iframe赋值方法
				window.parent.document.getElementById("batch_path_ids").value +=""+json.data.id+"_";
                //window.open()赋值方法
				//window.opener.document.getElementById("batch_path_ids").value = '';
				<?php } ?>				
