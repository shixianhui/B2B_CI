...
<table class="table_form" cellpadding="0" cellspacing="1">
  <caption>添加栏目</caption>
    ...
    <tr>
      <th width="20%"> <strong>关联的品牌</strong> <br/>
      <font color="red">[只对楼层分类起作用]</font>
	  </th>
      <td>
     <?php if ($brand_list) { ?>
     <?php foreach ($brand_list as $key=>$value) { ?>
     <label><input <?php if (in_array($value['id'], $brand_arr)) {echo "checked='checked'";} ?> class="checkbox_style" name="brand_ids[]" value="<?php echo $value['id']; ?>" type="checkbox"/> <?php echo $value['brand_name']; ?>[<?php echo $value['id']; ?>]</label>
     <?php }} ?>
      </td>
    </tr>
    <tr>
      <th width="20%"> <strong>关联的产品分类</strong> <br/>
      <font color="red">[只对楼层分类起作用]</font>
	  </th>
      <td>
     <?php if ($product_category_list) { ?>
     <?php foreach ($product_category_list as $key=>$value) { ?>
     <label><input <?php if (in_array($value['id'], $product_category_arr)) {echo "checked='checked'";} ?> class="checkbox_style" name="product_category_ids[]" value="<?php echo $value['id']; ?>" type="checkbox"/> <?php echo $value['product_category_name']; ?>[<?php echo $value['id']; ?>]</label>
     <?php }} ?>
      </td>
    </tr>
    <tr>
...
<script id="content" name="content" type="text/plain" style="width:800px;height:200px;"><?php if(! empty($menuInfo)){ echo html($menuInfo["content"]);}else{echo "";} ?></script>
