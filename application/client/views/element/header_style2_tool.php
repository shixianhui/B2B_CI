...
<div class="shop-head" <?php if ($store_info && $store_info['store_banner']) { ?> style="background:#f7f7f7 url(<?php echo $store_info['store_banner'];?>) no-repeat center center;"<?php } ?>>
	...
	<?php if ($store_info && $store_info['list_path_logo']) { ?>
		<a href="<?php echo getBaseUrl(false, '', "store/home/{$store_id}.html", $client_index); ?>"><img src="<?php echo $store_info['list_path_logo']; ?>" class="logo" style="display: none"></a>
    <?php } ?>
		<ul class="head-link">
			<li>
	...
	<div class="mall-menu">
		<div class="warp">
			<div class="all-category shop-category">
				<div class="title">
					<a href="<?php echo getBaseUrl(false, '', "store/product/{$store_id}.html", $client_index); ?>">全部热卖宝贝<i><Img src="css/default/<?php echo $style; ?>/images/shop_icon4.png"></i></a>
				</div>
				<div class="category" <?php if ($this->uri->segment(2) == 'home') { ?> style="display:block"<?php } ?>>
					<ul class="menu">
					<?php
					...
					?>
						<li cat_id="<?php echo $item['id']; ?>">
							<dl><dt><a style="color: #fff;" href="<?php echo $url; ?>"><?php echo $item['product_category_name']; ?></a></dt>
