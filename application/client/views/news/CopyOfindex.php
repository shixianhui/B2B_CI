...
<div id="container">
   <div class="banner980x60 mt5">
	<u>
	<?php
		$adList = $this->advdbclass->getOthorAd(7);
		if ($adList) {
    ...
    <?php echo $this->load->view('element/service_left_tool', '', TRUE); ?>
        ...
        <div class="right_content">
           ...
           <table width="100%" cellspacing="0" cellpadding="0" border="0" class="linktable" style="margin:0 auto;">
             <tbody>
             <?php if ($newsList) { ?>
			<?php foreach ($newsList as $news) {
				$url = getBaseUrl($html, "{$news['html_path']}/{$news['id']}.html", "news/detail/{$news['id']}.html", $client_index);
			?>
             <tr>
                <td width="80%" height="25" style="line-height:20px; border-bottom:1px #cccccc dashed; "><li><a title="<?php echo clearstring($news['title']); ?>" href="<?php echo $url; ?>"><font color="<?php echo $news['title_color']; ?>"><?php echo my_substr($news['title'], 40); ?></font></a></li></td>
                <td style="line-height:20px; border-bottom:1px #cccccc dashed;"><div align="center"><?php echo date('Y年m月d日', $news['add_time']); ?></div></td>
              </tr>
              <?php }} ?>
           </tbody>
           </table>
