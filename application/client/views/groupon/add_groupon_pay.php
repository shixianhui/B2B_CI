<section class="warp">
	<div class="cart_repeat mt25">
		<div class="bar clearfix">
			<h3 class="fl" style="line-height: 30px"><span class="icon"></span>请支付参团定金</h3>
            <span class="price fr">应付金额：<b class="red f18"><?php echo floatval($pintuan_info['deposit']); ?></b>元</span>
		</div>
        <P>商品名称：<?php echo $product_info['title']; ?></P>
	</div>
	<div class="cart_border mt30">
		<span class="tit">选择支付方式</span>
		<div class="border_d clearfix">
			<div class="cart_pay">
                <?php if ($user_info && $user_info['total'] >= $pintuan_info['deposit']){ ?><div class="balance_pay"><label><input type="checkbox" id="yue">余额支付 </label><span style="padding-left: 20px">账户余额 <b class="red"><?php if ($user_info) { echo $user_info['total'];}else{echo '0.00';}?></b> 元</span><div>支付密码：<input type="password" name="pay_password">
...
<script type="text/javascript">
    ...
    $("#goPay").click(function(){
        ...
        if($('#yue').attr("checked")){
            $.ajax({
                url : base_url+'index.php/groupon/my_yue_pay',
                type : 'post',
                data : {
                    record_id: '<?php if ($record_id) {echo $record_id;} ?>',
            ...
            location.href = base_url+'index.php/groupon/alipay_pay/<?php if ($record_id) {echo $record_id;} ?>.html';
        } else if(pay_type == 'weixin') {
            location.href = base_url+'index.php/groupon/my_pay_weixin/<?php if ($record_id) {echo $record_id;} ?>.html';
