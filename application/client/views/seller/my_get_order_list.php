...
<div class="member_tab mt20">
 <div class="hd">
 <ul>
	<li <?php if ($select_status == 'all') {echo 'class="on"';} ?> onclick="location.href = '<?php echo base_url().getBaseUrl(false, "", "{$template}/my_get_order_list/all.html", $client_index); ?>'">所有订单</li>
	<li <?php if ($select_status == 'a') {echo 'class="on"';} ?> onclick="location.href = '<?php echo base_url().getBaseUrl(false, "", "{$template}/my_get_order_list/a.html", $client_index); ?>'">待付款</li>
	<li <?php if ($select_status == 'b') {echo 'class="on"';} ?> onclick="location.href = '<?php echo base_url().getBaseUrl(false, "", "{$template}/my_get_order_list/b.html", $client_index); ?>'">待发货</li>
	<li <?php if ($select_status == 'c') {echo 'class="on"';} ?> onclick="location.href = '<?php echo base_url().getBaseUrl(false, "", "{$template}/my_get_order_list/c.html", $client_index); ?>'">待收货</li>
	<li <?php if ($select_status == 'd') {echo 'class="on"';} ?> onclick="location.href = '<?php echo base_url().getBaseUrl(false, "", "{$template}/my_get_order_list/d.html", $client_index); ?>'">待评价</li>
</ul>
 </div>
 <div class="bd">
 <div class="clearfix">
     <form class="product_search" id="product_search" method="post" action="index.php/<?php echo $template; ?>/my_get_order_list.html">
         <div style="padding-top: 20px;">
             ...
             <span>收货人姓名：</span><input style="width:100px;" type="text" name="user_name">
             <span>申请时间：</span><input type="text" name="add_time_start" id="add_time_start" style="width: 80px;" autocomplete="off">--<input type="text" name="add_time_end" id="add_time_end" style="width: 80px" autocomplete="off">
...
?>
  ...
  <tr>
	...
	?>
    <tr>
    <td width="36%" valign="middle"><div class="info"><a href="<?php echo $url; ?>" target="_blank"><img src="<?php if ($od_value['path']) { echo preg_replace('/\./', '_thumb.', $od_value['path']);}else{echo 'images/default/load.jpg';} ?>"><?php echo $od_value['product_title']; ?>
    ...
    <?php if($od_key == 0) { ?>
    <td rowspan="<?php echo count($value['order_detail_list']); ?>" width="15%" align="center"><span class="red"><small>¥</small><?php echo $value['total']; ?></span>
 <br/>
    ...
    <td rowspan="<?php echo count($value['order_detail_list']); ?>" width="14%" align="center" >
        ...
        <?php  if($value['exchange_status'] != ''){ ?><font class="c9 mt5"><?php echo $exchange_status_arr[$value['exchange_status']]; ?></font><br><?php } ?>
<!--        <a href="--><?php //echo $view_url; ?><!--"><font class="c9 mt5">查看详情</font></a>-->
    </td>
    <td rowspan="<?php echo count($value['order_detail_list']); ?>" width="14%" align="center">
        <?php if ($value['status'] == 0) { ?>
            <a style="display: none" onclick="change_pay(<?php echo $value['id']; ?>,'<?php echo $value['order_number']; ?>','<?php echo date('Y-m-d H:i:s', $value['add_time']); ?>','<?php echo $value['total']; ?>');" href="javascript:void(0);" title="用于线下付款修改订单状态" class="m_btn">改为已付款</a>
            <a onclick="change_price(<?php echo $value['id']; ?>,'<?php echo $value['order_number']; ?>','<?php echo date('Y-m-d H:i:s', $value['add_time']); ?>','<?php echo $value['total']; ?>');" class="m_btn" href="javascript:void(0);">修改金额</a>
            <a onclick="close_order(<?php echo $value['id']; ?>,'<?php echo $value['order_number']; ?>','<?php echo date('Y-m-d H:i:s', $value['add_time']); ?>','<?php echo $value['total']; ?>');" class="m_btn" href="javascript:void(0);">交易关闭</a>
        <?php } else if ($value['status'] == 1) { ?>
            <a onclick="delivery(<?php echo $value['id']; ?>,'<?php echo $value['order_number']; ?>','<?php echo date('Y-m-d H:i:s', $value['add_time']); ?>','<?php echo $value['total']; ?>');" class="m_btn" href="javascript:void(0);">发货</a><br/>
        <?php } else if ($value['status'] == 2) {  ?>
            <a onclick="receiving(<?php echo $value['id']; ?>,'<?php echo $value['order_number']; ?>','<?php echo date('Y-m-d H:i:s', $value['add_time']); ?>','<?php echo $value['total']; ?>');" class="m_btn" href="javascript:void(0);">确认收货</a>
        <?php } else { ?>
            <a href="<?php echo $url; ?>" class="m_btn">再次购买</a>
        <?php } ?>
    </td>
    <?php } ?>
  </tr>
