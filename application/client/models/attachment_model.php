...
class Attachment_model extends CI_Model {
    ...
    public function getRandInfo($ids = NULL) {
		$ret = array();				
	    $query = $this->db->query("SELECT * FROM attachment where id in ({$ids}) ORDER BY RAND() LIMIT 1");
	    if ($query->num_rows() > 0) {
			$ret = $query->result_array();
			$ret = $ret[0];
		}
    ...
    public function gets4($ids = '') {
        $ret = array();
        $query = $this->db->query("select * from {$this->_tableName} where id in ({$ids}) order by field(id, {$ids}) limit 1");
        if ($query->num_rows() > 0) {
            $ret = $query->result_array();
        }
