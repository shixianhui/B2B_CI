...
class Product_model extends CI_Model {
    ...
    public function gets2($strWhere = NULL, $limit = NULL, $offset = NULL, $by = 'id', $order = 'DESC') {
    	$ret = array();
    	$this->db->select("{$this->_tableName}.*,{$this->_storeTName}.store_name");
    	if ($order == 'all') {
    		$this->db->order_by("{$this->_tableName}.sort", 'DESC');
    		$this->db->order_by("{$this->_tableName}.id", $order);
    	} else {
    		$this->db->order_by("{$this->_tableName}.{$by}", $order);
    	}
    ...
    public function getColor($strWhere = '', $limit = "limit 10") {
        ...
        $ret = array();
        $query = $this->db->query("select color_name from product_size_color where product_id in ({$ids}) group by color_name {$limit}");
        if ($query->num_rows() > 0) {
    ...
    public function getSize($strWhere = '', $limit = "limit 10") {
        ...
        $ret = array();
        $query = $this->db->query("select size_name from product_size_color where product_id in ({$ids}) group by size_name {$limit}");
        if ($query->num_rows() > 0) {
            $bmpRet = $query->result_array();
            foreach ($bmpRet as $key => $value) {
    ...
    public function getIdsBySize($strWhere = '', $limit = '') {
        $ids = '';
        $query = $this->db->query("select product_id from product_size_color where size_name = '{$strWhere}' group by product_id {$limit}");
        if ($query->num_rows() > 0) {
            $ret = $query->result_array();
            foreach ($ret as $value) {
    ...
    public function getIdsByColor($strWhere = '', $limit = '') {
        $ids = '';
        $query = $this->db->query("select product_id from product_size_color where color_name = '{$strWhere}' group by product_id {$limit}");
        if ($query->num_rows() > 0) {
            $ret = $query->result_array();
            foreach ($ret as $value) {
                $ids .= $value['product_id'] . ',';
    ...
    public function getProductStock($product_id = 0, $color_id = 0, $size_id = 0) {
        $ret = array();
        $query = $this->db->query("select stock, price, product_num, color_name, size_name from product_size_color where product_id = {$product_id} and color_id = {$color_id} and size_id = {$size_id} limit 1");
        if ($query->num_rows() > 0) {
            $ret = $query->result_array();
        }
    ...
    public function getPrice($productId) {
        $ret = array();
        $query = $this->db->query("select min(price) as min_price, max(price) as max_price from product_size_color where product_id = {$productId}");
        if ($query->num_rows() > 0) {
            $ret = $query->result_array();
        }
    ...
    public function getDetailPrice($productId = 0) {
        $ret = array();
        $query = $this->db->query("select * from product_size_color where product_id = {$productId} order by id ASC");
        if ($query->num_rows() > 0) {
            $bmpRet = $query->result_array();
            foreach ($bmpRet as $key => $value) {
                $ret[$key] = $value;
