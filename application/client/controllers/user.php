...
//        '4' => '原路返回退款成功'
    ...
    public function __construct() {
        ...
        $this->load->model('Sms_email_model', '', TRUE);
    ...
    public function index() {
        ...
        //订单
        $item_list = $this->Orders_model->gets('*', "user_id = " . get_cookie('user_id'), 5, 0);
...
// 	        }
            ...
            $todayScoreTotal = $this->Score_model->getTodayTotal($userInfo['id']);
            if ($todayScoreTotal + $score_setting_info['login_score'] <= $score_setting_info['login_score_max']) {
                $balance = $userInfo['score'] + $score_setting_info['login_score'];
    ...
    public function get_reg_sms_code() {
        if ($_POST) {
            ...
            if ($type == 'reg') {
                ...
                $count = $this->User_model->rowCount(array("mobile" => $mobile));
                if ($count) {
    ...
    public function my_change_user_info() {
        ...
        $item_info = $this->User_model->get('*', array('user.id' => get_cookie('user_id')));
        if ($_POST) {
...
//            }
    ...
    public function my_change_mobile() {
        ...
        checkLogin();
        $systemInfo = $this->System_model->get('*', array('id' => 1));
        $user_info = $this->User_model->get('*', "id = " . get_cookie('user_id'));
        if ($_POST) {
            ...
            $result = $this->User_model->save(array('mobile' => $mobile), array('id' => get_cookie('user_id')));
            if ($result) {
    ...
    public function my_change_email() {
        ...
        if ($_POST) {
            ...
            $result = $this->User_model->save(array('email' => $email, 'is_check_email' => 1), array('id' => get_cookie('user_id')));
            if ($result) {
                printAjaxSuccess(getBaseUrl(false, '', 'user/my_change_user_info.html', $systemInfo['client_index']), '修改成功');
            } else {
                printAjaxError('fail', '修改失败');
    ...
    public function send_email() {
        if ($_POST) {
            ...
            $count = $this->User_model->rowCount(array('email' => $email));
            if ($count > 0) {
...
EOT;
    ...
    public function my_change_pass() {
        ...
        if ($_POST) {
            ...
            //验证密码是否正确
            $user_info = $this->User_model->get('password, username', array('user.id' => get_cookie('user_id')));
            if (!$user_info) {
    ...
    public function my_change_pay_pass() {
        ...
        $item_info = $this->User_model->get('pay_password,username', array('user.id' => get_cookie('user_id')));
        if ($_POST) {
    ...
    public function my_get_favorite_list($page = 0) {
        ...
        $strWhere = array('favorite.user_id' => get_cookie('user_id'), 'type' => 'product');
        //分页
    ...
    public function my_favorite_store_list($page = 0) {
        ...
        checkLogin();
        $systemInfo = $this->System_model->get('*', array('id' => 1));
        $strWhere = array('favorite.user_id' => get_cookie('user_id'), 'type' => 'store');
        //分页
    ...
    public function my_delete_favorite() {
        ...
        if ($_POST) {
            ...
            if ($this->Favorite_model->delete(array('id' => $id, 'user_id' => get_cookie('user_id'), 'type' => $type))) {
...
//                }
    ...
    public function my_save_exchange($orders_detail_id = NULL) {
        ...
        $systemInfo = $this->System_model->get('*', array('id' => 1));
        //当前位置
        $location = "<a href='index.php/user.html'>会员中心</a> > <a href='javascript:void(0);'>我是消费者</a> > <a href='javascript:void(0);'>我的订单</a> > 退款";
    ...
    public function my_view_exchange($orders_detail_id = NULL) {
        ...
        $item_info = $this->Orders_model->get('*', "id = {$exchange_info['orders_id']} and user_id = {$user_id} and status in (1,2,3) ");
        if (!$item_info) {
            $data = array(
                'user_msg' => '此订单信息不存在',
                'user_url' => $prfUrl
    ...
    public function my_save_address($id = NULL) {
        ...
        if ($_POST) {
            ...
            if ($is_default == 1) {
                $this->User_address_model->save(array('is_default' => 0), array('user_id' => get_cookie('user_id'), 'is_default' => 1));
            }
    ...
    public function my_score_list($page = 0) {
        ...
        $user_info = $this->User_model->get('score', array('user.id' => get_cookie('user_id')));
    ...
    public function my_financial_list($type = 'all', $page = 0) {
        ...
        $user_info = $this->User_model->get('total', array('user.id' => get_cookie('user_id')));
    ...
    public function my_comment_save($order_id = 0) {
        ...
        $order_info = $this->Orders_model->get('*', array('id' => $order_id, 'user_id' => get_cookie('user_id'), 'status' => 3, 'is_comment_to_seller' => 0));
        if (empty($order_info)) {
            $data = array(
                'user_msg' => '不存在要评价的订单',
        ...
        $user_info = $this->User_model->get('username,nickname', array('id' => get_cookie('user_id')));
        if ($_POST) {
...
//            }
    ...
    public function my_comment_view($order_id = 0) {
        ...
        $order_id = intval($order_id);
        $order_info = $this->Orders_model->get('*', array('id' => $order_id, 'user_id' => get_cookie('user_id'), 'status' => 3, 'is_comment_to_seller' => 1));
        if (empty($order_info)) {
            $data = array(
                'user_msg' => '不存在已评价的订单',
                'user_url' => base_url() . "index.php/order/my_order_index.html"
        ...
        $comment_store_info = $this->Comment_store_model->get('*', array('order_id' => $order_id, 'user_id' => get_cookie('user_id')));
        $data = array(
            'site_name' => $systemInfo['site_name'],
            'index_name' => $systemInfo['index_name'],
            'index_url' => $systemInfo['index_url'],
    ...
    //我的拼团活动
    public function my_group_purchase() {
        //判断是否登录
        checkLogin();
        $user_id = get_cookie('user_id');
        $systemInfo = $this->System_model->get('*', array('id' => 1));
        $group_purchase_list = $this->Ptkj_record_model->gets(array('user_id' => get_cookie('user_id')));
        $ids = '';
        ...
        $layout = array(
            'content' => $this->load->view('user/my_group_purchase', $data, TRUE)
        );
        $this->load->view('layout/user_layout', $layout);
        //缓存
        if ($systemInfo['cache'] == 1) {
    ...
    public function my_flash_sale() {
        ...
        $user_id = get_cookie('user_id');
        $systemInfo = $this->System_model->get('*', array('id' => 1));
        $flash_sale_info = $this->Flash_sale_record_model->gets(array('user_id' => get_cookie('user_id')));
    ...
    public function my_read_message() {
        ...
        if ($_POST) {
            ...
            $ret = $this->Message_model->save(array('is_read' => 1), array('id' => $id, 'to_user_id' => get_cookie('user_id')));
            if (!$ret) {
                printAjaxError('fail', '操作失败');
            }
            printAjaxSuccess('success', '操作成功');
    ...
    public function my_delete_message() {
        ...
        if ($_POST) {
            ...
            $ret = $this->Message_model->delete("id in ($ids) and to_user_id = " . get_cookie('user_id'));
            if (!$ret) {
                printAjaxError('fail', '删除失败');
            }
            printAjaxSuccess('success', '删除成功');
    ...
    public function alipay_pay_recharge() {
        ...
        $out_trade_no = $this->_get_unique_pay_log_number();
        $total_fee =  $money;
        //生成充值记录
        if(!$this->Pay_log_model->rowCount(array('out_trade_no'=>$out_trade_no, 'payment_type'=>'alipay', 'order_type'=>'recharge'))) {
            $fields = array(
                'user_id'=>       $user_id,
                'total_fee'=>     $money,
                'total_fee_give'=>0,
                    ...
                    if ($pay_log_info && $alipay_config['seller_id'] == $seller_id && $total_fee == $pay_log_info['total_fee']) {
                        if ($pay_log_info['trade_status'] != $trade_status && $pay_log_info['trade_status'] != 'TRADE_FINISHED' && $pay_log_info['trade_status'] != 'TRADE_SUCCESS' && $pay_log_info['trade_status'] != 'TRADE_CLOSED') {
                            ...
                            if ($this->Pay_log_model->save($fields, array('id' => $pay_log_info['id']))) {
                                ...
                                if ($user_info) {
                                    //财务记录
                                    if (!$this->Financial_model->rowCount(array('type' => 'recharge_in', 'ret_id' => $pay_log_info['id'], 'user_id'=>$pay_log_info['user_id']))) {
    ...
    private function first_auth_login($auth_type = '', $exist = array(), $nickname = '', $headimgurl = '', $sex = 0, $unique_str = '', $is_mobile = false) {
        if (!$exist) {
            ...
            $fields = array(
                ...
                'add_time' => $addTime,
                'nickname' => $nickname,
                'path' => empty($headimgurl) ? '' : $headimgurl,
                'sex' => $sex,
            );
            switch ($auth_type) {
                case 'wechat' :
...
//            }while($this->User_model->rowCount2(array('user_num'=>$user_num)) ? true : false);
//            $fields['user_num'] = $user_num;
    ...
    private function _createPopCode() {
        ...
        $count = $this->User_model->rowCount(array("pop_code" => $pop_code));
        if ($count) {
            $i = 0;
            while ($i < 100) {
                $i++;
