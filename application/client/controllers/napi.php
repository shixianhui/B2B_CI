...
class Napi extends CI_Controller {
    private $_app_id = '2016122104489543';//2016122104489543
    private $_rsaPrivateKey = '';
    //支付宝公钥，不要用应用公钥
    private $_alipayrsaPublicKey = '';
    ...
    public function login() {
        if ($_POST) {
            ...
            printAjaxData($this->_tmp_user_info($user_info['id'], $session_id, $push_cid));
        }
    }
...
//小程序登录
    public function wx_login(){
        if ($_POST){
            ...
            $appid = 'wx44b8cb9a4610f76d';
            $appSecret = '10b891cf7f024119a9bcbd222d92cae9';
            $json = file_get_contents("https://api.weixin.qq.com/sns/jscode2session?appid={$appid}&secret={$appSecret}&js_code={$code}&grant_type=authorization_code");
            $obj = json_decode($json);
            if (isset($obj->errmsg)) {
                printAjaxError('fail', 'invalid code!');
        ...
        if ($user_info) {
            ...
            $todayScoreTotal = $this->Score_model->getTodayTotal($user_info['id']);
            if ($todayScoreTotal + $score_setting_info['login_score'] <= $score_setting_info['login_score_max']) {
                $balance = $user_info['score'] + $score_setting_info['login_score'];
...
//            }
            ...
            $sms_content = "【蚁立网】您的验证码是：{$verify_code} 请不要把验证码泄露给其他人。如非本人操作，可不用理会！";
            /*             * *************************半小时限制**************************** */
    ...
    public function get_user_info() {
        $user_id = $this->_check_login();
        $user_info = $this->User_model->get('id,username,path,nickname,total,score,sex,real_name,mobile,id_card,is_id_card_auth', array('id' => $user_id));
        if ($user_info) {
            //头像
        ...
        $store_info = $this->Store_model->get('id,display', array('user_id'=>$user_id));
        $user_info['store_id'] = $store_info ? $store_info['id'] : 0;
        $user_info['store_display'] = $store_info ? $store_info['display'] : 0;
        $strWhere = array('user_id'=>$user_id);
        $user_info['order_count'] = $this->Orders_model->rowCount($strWhere);
        $strWhere_0 = array('user_id'=>$user_id, 'status'=>0);
        $user_info['order_count_0'] = $this->Orders_model->rowCount($strWhere_0);
        $strWhere_1 = array('user_id'=>$user_id, 'status'=>1);
        $user_info['order_count_1'] = $this->Orders_model->rowCount($strWhere_1);
        $strWhere_2 = array('user_id'=>$user_id, 'status'=>2);
        $user_info['order_count_2'] = $this->Orders_model->rowCount($strWhere_2);
        $strWhere_3 = array('user_id'=>$user_id, 'status'=>3);
        $user_info['order_count_3'] = $this->Orders_model->rowCount($strWhere_3);
     ...
     * @return json
     */
    public function get_product_detail($id = 0){
        $item_info = $this->Product_model->get('title,sell_price,batch_path_ids,app_content,color_size_open,product_color_name,product_size_name,store_id,weight,brand_name,material_name,style_name,product_num,stock,unclear_price', array('id' => $id));
        if (!$item_info) {
        ...
        $item_info['app_content'] = filter_content(html($item_info['app_content']),base_url());
        preg_match_all('/<\s*img\s+[^>]*?src\s*=\s*(\'|\")(.*?)\\1[^>]*?\/?\s*>/i', $item_info['app_content'], $arr);
        if (isset($arr[2]) && $arr[2]) {
            foreach ($arr[2] as $value) {
                if (!preg_match("/^http/", $value)) {
                    $item_info['app_content'] = str_replace($value, base_url() . $value, $item_info['app_content']);
                }
            }
        }
        $item_info['attachment_list'] = $attachment_list;
        //规格属性是否开启
        if ($item_info['color_size_open'] == 1) {
            //规格名称[color]
            $color_list = $this->Product_size_color_model->gets('id,color_id,color_name, color_name_hint, path', array('product_id' => $id), 'color_id');
            if ($color_list) {
                foreach ($color_list as $key => $ls) {
                    $tmp_image_arr = $this->_fliter_image_path($ls['path']);
                    $color_list[$key]['path'] = $tmp_image_arr['path'];
                    $color_list[$key]['path_thumb'] = $tmp_image_arr['path_thumb'];
                }
            }
            $item_info['color_list'] = $color_list;
            //规格名称[size]
            $size_list = $this->Product_size_color_model->gets('id,size_id,size_name, size_name_hint', array('product_id' => $id), 'size_id');
            $item_info['size_list'] = $size_list;
        }
        //评论列表
        $comment_list = $this->Comment_model->gets('username,content,user_id,add_time', array('product_id' => $id), 10, 0);
        if ($comment_list) {
            foreach ($comment_list as $key => $ls) {
                $user_info = $this->User_model->get('path', array('id' => $ls['user_id']));
                $tmp_image_arr = $this->_fliter_image_path($user_info['path']);
                $comment_list[$key]['username'] = hideStar($ls['username']);
                $comment_list[$key]['add_time'] = date('Y-m-d', $ls['add_time']);
                $comment_list[$key]['path'] = $tmp_image_arr['path'];
                $comment_list[$key]['path_thumb'] = $tmp_image_arr['path_thumb'];
            }
        }
        $item_info['comment_list'] = $comment_list;
        //是否收藏
        $favorite_info = 0;
        $user_id = $this->session->userdata("user_id");
        if ($user_id) {
            $favorite_info = $this->Favorite_model->get('id', array('favorite.type' => 'product', 'favorite.item_id' => $id, 'favorite.user_id' => $user_id)) ? 1 : 0;
            //浏览记录
            $this->Browse_model->save(array('user_id'=>$user_id,'type'=>'product','item_id'=>$id,'add_time'=>time()));
        }
        $item_info['favorite_info'] = $favorite_info;


        // 最大ID
        $max_id = $this->Comment_model->get_max_id(array('product_id' => $id));
        //是否有下一页
        $cur_count = count($comment_list);
        $total_count = $this->Comment_model->rowCount(array('product_id' => $id));
        $is_next_page = 0;
        if ($total_count > $cur_count) {
            $is_next_page = 1;
        }
        $item_info['max_id'] = $max_id;
    ...
    public function get_store_list($auth_type = 0, $max_id = 0, $since_id = 0, $per_page = 1, $page = 1) {
        ...
        if ($_POST) {
            ...
            if($style_id){
                ...
                $strWhere .= " and id in (select store_id from product where style_name = '{$style_info['style_name']}')";
            }
...
//            }
     ...
     * 获取首页广告
     */
    public function get_index_ad_list($id = 1){
...
//            }
     ...
     * 文章列表
     */
    public function get_page_list($id = NULL){
        $item_list = $this->Page_model->gets2('id,title',array('category_id'=>$id));
        printAjaxData(array('item_list' => $item_list));
    }
    ...
    private function _order_weixin_pay($item_info = NULL) {
        ...
        $unifiedOrder->SetBody('蚁立网付款'); //商品或支付单简要描述
...
//             }
    ...
    private function _tmp_user_info($user_id = NULL, $session_id = 0, $push_cid = '') {
        ...
        $user_info = $this->User_model->get('id,username,mobile,path,nickname,total,score,sex,real_name', array('id' => $user_id));
    ...
    private function _check_login($is_check_store = false) {
        ...
        $user_id = $this->session->userdata("user_id");
        $item_info = $this->User_model->get('display,seller_group_id', array('id' => $user_id));
        if (!$item_info) {
        ...
        if ($is_check_store) {
            $seller_group_info = $this->Seller_group_model->get('user_id',array('id' => $item_info['seller_group_id']));
            $user_id = empty($seller_group_info) ? $user_id : $seller_group_info['user_id'];
            $storeInfo = $this->Store_model->get('display,close_reason', array('user_id' => $user_id));
            if (!$storeInfo) {
                printAjaxError('fail', '您还未申请店铺,请申请店铺');
            }
            if ($storeInfo['display'] == 0) {
                printAjaxError('fail', '店铺审核中，请耐心等待');
            } else if ($storeInfo['display'] == 2) {
                printAjaxError('fail', '店铺审核暂未通过，原因：' . $storeInfo['close_reason']);
            } else if ($storeInfo['display'] == 3) {
                printAjaxError('fail', '店铺已关闭，原因：' . $storeInfo['close_reason']);
            }
        }
        return $user_id;
    }
